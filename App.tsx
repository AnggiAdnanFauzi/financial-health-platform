...
import { 
  ...
  apiSaveDiagnostic, apiGetHistories, apiClearHistories, apiUploadAvatar,
...
type ViewType = 'landing' | 'input' | 'dashboard' | 'report' | 'insights' | 'simulation' | 'profile' | 'login' | 'register' | 'admin';

const getViewFromPath = (path: string): ViewType => {
  const clean = path.replace(/^\/+|\/+$/g, '').toLowerCase();
  const validViews: ViewType[] = ['input', 'dashboard', 'report', 'insights', 'simulation', 'profile', 'login', 'register', 'admin'];
...
const AppContent: React.FC = () => {
  ...
  const [view, setView] = useState<ViewType>(() => {
    if (typeof window !== 'undefined') {
      ...
      const validViews: ViewType[] = ['landing', 'input', 'dashboard', 'report', 'insights', 'simulation', 'profile', 'login', 'register', 'admin'];
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    if (apiRes) {
      if (apiRes.success && apiRes.user) {
        ...
        showToast(role === 'admin' ? 'Login Admin berhasil!' : AUTH_TEXT[lang].loginSuccess, 'success');
        setLoginEmail('');
        setLoginPassword('');
        if (role === 'admin') {
          setView('admin');
